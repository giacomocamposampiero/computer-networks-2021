...
int main()
{
    ...
    while(1){
        ...
        char command_cpy[100];
        strcpy(command_cpy, request);
        ...
        if(!strcmp(path, "/reflect")) {
            FILE *outf;
            outf = fopen("reflect.txt", "w+");
            fputs(command_cpy, outf);
            fputs("\r\n", outf);
            for(i=1; i<k; i++) {
                char header[100];
                sprintf(header, "%s:%s\r\n", h[i].n, h[i].v);
            ...
            fputs("\r\n", outf);
            char port[8];
            sprintf(port, "%d", remote_addr.sin_port);
            fputs(port, outf);
            fclose(outf);
            strcpy(path+1,"reflect.txt");	
        }
