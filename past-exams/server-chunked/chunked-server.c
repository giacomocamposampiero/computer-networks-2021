...
int main() {
    ...
    addr.sin_port = htons(42068);   // service port : 42069
    ...
    while(1) {
        ...
        if ((fin = fopen(path+1,"rt"))==NULL) {
            ...
            // each chunk correspond to a line of the server
            char* lines[50];
            int num_lines = 0;
            int line_size = 1000;
            ...
            do {
                ...
                lines[num_lines] = fgets(line, line_size, fin);
                line[strcspn(line, "\n")] = 0;
            } while(lines[num_lines++] != NULL);
            // send the content
            char length[10];
            for(int i=0; i<num_lines-1; i++) {
                //write the chunk size as hexadecimal
                sprintf(length, "0x%x\r\n", (unsigned int) strlen(lines[i]));
                write(s2, length, strlen(length));
                // write the chunk body
                write(s2, lines[i], strlen(lines[i]));
                write(s2, "\r\n", 2);
                // delocate line memory
                free(lines[i]);
            }
            // write last chunk
            write(s2, "0x0\r\n\r\n", 11);
